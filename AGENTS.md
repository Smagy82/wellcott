...
- **Офлайн**: 10 429 клиник HRSA + 11 992 MH-учреждений SAMHSA в SQLite (`assets/clinics-v7.db`),
...
## Правила данных — КРИТИЧНО, читать перед любым бейджем или фильтром

⛔ **ЗАПРЕЩЕНО** выводить медицинские услуги эвристикой (скрейпинг сайтов,
поиск слов в тексте, вывод по организации на локацию).
Бейдж услуги ставится ТОЛЬКО из первоисточника, где услуга указана ДЛЯ КОНКРЕТНОЙ ЛОКАЦИИ.
**Нет данных → нет бейджа. Никаких компромиссов с кавэйтами.**

Почему: люди доверяют фильтру, который поставили мы. Человек с зубной болью
фильтрует "Dental", едет 40 миль, слышит "мы этим не занимаемся" →
убитое доверие, плохой отзыв в сторе. Ложный бейдж ХУЖЕ отсутствия фичи.

Прецедент (откат 14.07.2026): dental-фильтр через скрейпинг сайтов клиник
(52% "strong" = org-level signal → 82 адреса arcare.net, дантист не везде).
Детальный разбор: `IDEAS.md` → "Dental через HRSA/UDS — ТУПИК, ЗАКРЫТО".
...
### ⛔ Регрессионный тест при смене источника данных

При смене источника данных или пересборке базы ОБЯЗАТЕЛЬНО прогнать регрессионный тест: сравнить выдачу по контрольному городу (Fort Wayne, 25 миль) со старой базой. Если ЛЮБАЯ известная клиника исчезла — источник отклоняется, независимо от общего числа записей.
